...
public class Main {
	...
	public static void addMainPanel(Question questions[], User client, int caller) {
		if(caller == 0)
		{
				...
				while(true) {
					if(loginDisabled == false) {
						...
						if(loginDisabled == true) {
							addMainPanel(login.questions.questions, login.client, 0);
							mainDisabled = false;
						}
					}
						...
						if(executorDisabled == true) {
							addMainPanel(mpanel.p1.qlist.questions, mpanel.p1.client, 1);
							mainDisabled = false;
						}
					}
					try {
